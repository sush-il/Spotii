...
const DisplayAll: React.FC<{ title: string; data: itemProp[] }> = ({
  title,
  data
}) => {
  ...
  const getData = async () => {
    ...
    try {
      const getDatabyTimeFrame = await fetch(
        `http://localhost:5000/${dataTypeToGet}/?code=${accessToken}&timeframe=${timeframe}`
      );
            ...
            >
              <Link to={urlToDirect}>
                <img
                  ...
                  style={{ width: '7rem', marginBottom: '0.5rem' }}
                  alt="Cover"
                />
              </Link>
