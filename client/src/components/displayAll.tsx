...
const DisplayAll: React.FC<{accessToken:string; title: string; data: allDataProps[]}> = ({accessToken, title, data }) => {
  const url = new URL(window.location.href).pathname;
  const dataTypeToGet = url==="/artists" ? "getTopArtists" : "getTopTracks"
  const [dataUsed, setDataUsed] = useState<allDataProps[]>(data);
